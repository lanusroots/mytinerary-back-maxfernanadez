...
async function createCities(arrayCities){
    try{
        ...
        for(let city of arrayCities) {
            let user = await User.findOne({ mail: city.admin_id })      // Busco el USER q coincida el MAIL del objeto
            let admin_id = await user._id                               // SOLO necesito el id de ese USER
            city.admin_id = admin_id                                    // Reasigno el valor del ADMIN_ID de cada CITY para cambiar el MAIL
            await City.create(city)                                     // q viene por default por el q corresponda (ID).
        }
        console.log('Done!')
    } catch (error) {
        console.log(error)
